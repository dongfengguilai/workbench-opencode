...
技术状态为 READY_FOR_USER_ACCEPTANCE。release 检查仍因 A14 未签收和历史 Qwen 平台比较没有平台因果收益而返回阻止；这是预期状态。历史结果不会被改写，本次通过的是单独冻结的固定 Luna 产品配置。用户签收后只更新该产品目标的门禁记录；签收前不得标记 DELIVERED，不得发布或进入 v4。
