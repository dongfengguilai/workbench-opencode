# 当前交付状态：REMOTE_RETIRED（本机继续开发）

2026-09-18 按用户要求，已停止并清除 `10.243.117.57` 的 WorkBench 试验部署。用户已确认真实内网登录可用；这是用户反馈，不扩写为双身份、全部功能验收或 A10 已签收。后续默认在本机开发，下一个大版本明确要求部署时才再次使用远程服务器。

- 用户现在能做：在现有本机源码与 GitHub 仓库继续开发，使用本机已保留的固定离线部署包；远程试用成果、会话、截图、账号配置、私有 CA 和历史备份均已完整冷备到本机。未提交参考图 `web_ui_example_png/` 保留，不入提交。
- 用户现在不能做：访问旧远程工作台 8443／8447 或预览 8445／8449；它们已主动退役。远程安装目录、15 个专属容器、4 个专属网络、两套限额挂载与 loop 关联及已核对的旧升级暂存目录已清除。没有重新初始化、伪造登录、运行模型任务或继续功能验收。
- 冷备位置：`/home/vmware/Workspace/projects/experiments/Workbench_space/releases/retired-workbench-v1/20260918T053225Z/.WorkBench-v1-retirement-20260918T053225Z.tar.gz`，2,303,035,000 字节，SHA-256 `b282dccfc45a19b6786598d4e943b89ef0693b39df3d27f21e38440edce4a124`。目录 0700、归档 0600；含模型凭据、管理员密码摘要与 CA 私钥，不能上传 GitHub。已核对远端与本机完整摘要，读取全部 1,319 个普通文件并核对工程师、管理员磁盘镜像；未在运行服务器上试恢复。
- 保护证据：停止前两原生任务均空闲；冷备记录了两项目、原生状态、Git 索引和计数摘要。Qwen 及其他业务的容器 ID、镜像、启动时间、运行状态与重启计数前后一致；共享 Node 镜像保留，不使用全局 prune，不修改宿主磁盘／网络设置或用户电脑证书。
- 真实证据：`opencode-cloud/evidence/retirement-local-backup-verification.json`、`retirement-server-result.json`、`retirement-postcheck.json`、`retirement-upgrade-staging-inspection.json`。首次 du 权限中止、Docker 挂载顺序误判和短网络 ID 比较失败均单独保存；均发生在删除前，修正维护者检查方式后继续，未放宽保护范围。
- 下一步只处理：本机下一大版本开发；当前不继续远程验收或重新部署。未来部署时重新确定域名与证书方案，再执行该版本真实验收；A10 仍由用户签收。用户电脑的旧根证书本轮未卸载，删除远程服务器文件不会自动删除客户端信任。

## 退役前交付状态（历史记录，不代表当前远程可用）
