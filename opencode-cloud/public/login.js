async function clearStorage(){document.querySelector('#login').dataset.phase='storage';localStorage.clear();sessionStorage.clear();document.querySelector('#login').dataset.phase='indexeddb';for(const db of await indexedDB.databases())if(db.name)indexedDB.deleteDatabase(db.name);document.querySelector('#login').dataset.phase='cache';for(const name of await caches.keys())await caches.delete(name);document.querySelector('#login').dataset.phase='serviceworker';for(const worker of await navigator.serviceWorker.getRegistrations())await worker.unregister();}
