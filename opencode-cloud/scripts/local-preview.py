...
entry=root/'src/local-preview.mjs'
record=root/'runtime/local-preview-process.json'
def owned():
    try:
...
def healthy():
    try:
        ...
        opener.open('http://localhost:8445/',timeout=2)
        return False
    except urllib.error.HTTPError as error:return error.code==401
    except OSError:return False
action=sys.argv[1] if len(sys.argv)==2 else ''
if action not in ('start','stop','status'):sys.exit('Usage: local-preview.py start|stop|status')
if action=='stop':
    pid=owned()
    if pid:
    ...
    print('http://localhost:8445 '+('ready' if ok else 'not ready'))
    ...
    if owned():
        ...
        with socket.socket() as probe:
            probe.settimeout(1)
            if probe.connect_ex(('127.0.0.1',8445))==0:
                sys.exit('Port 8445 is already occupied by an unowned listener; nothing was stopped or replaced')
        log=root/'runtime/local-preview.log'
        ...
        for _ in range(30):
            if child.poll() is not None:
                record.unlink(missing_ok=True);sys.exit('Relay failed; see protected runtime/local-preview.log')
            if healthy():break
            time.sleep(.1)
        else:sys.exit('Relay started, but verified upstream is not ready; inspect runtime/local-preview.log')
    print('Local browser ready: http://localhost:8445')
