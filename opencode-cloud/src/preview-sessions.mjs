...
export class PreviewSessions {
 ...
 claim(token){const key=digest(token||'');const entry=this.tickets.get(key);this.tickets.delete(key);if(!entry||entry.expires<=this.now()||!this.parent(entry.hash))return;const credential=randomBytes(32).toString('base64url');this.credentials.set(digest(credential),{hash:entry.hash,expires:this.now()+this.credentialTtl});return credential;}
