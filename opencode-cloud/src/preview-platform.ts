...
import {previewOrigin,workbenchOrigin,formalOrigins,previewCookieName} from './preview-origin.mjs';
export function previewPlatform({parent,register,closeSession,relayKey}:{parent:(hash:string)=>any,register:(hash:string)=>any,closeSession:(hash:string)=>void,relayKey:()=>string}){
 ...
 function identity(req:any){if(!sameSecret(req.headers['x-workbench-preview-relay'],relayKey()))return;const name=previewCookieName(req.headers['x-workbench-preview-origin']);if(!name)return;const id=access.current(readCookie(req.headers.cookie,name));return id&&req.headers['x-workbench-preview-origin']===previewOrigin(id.username)?id:undefined;}
 function registrations(req:any,id:any){return (close:()=>void)=>{const a=register(id.hash)(close);const b=register(access.connectionKey(readCookie(req.headers.cookie,previewCookieName(req.headers['x-workbench-preview-origin']))))(close);return()=>{a();b();};};}
 ...
 return {
  ...
  handle(req:any,res:any){
   ...
   if(url.pathname==='/__preview/__workbench/claim'&&req.method==='GET'){
    ...
    res.writeHead(303,{'Location':'/','Cache-Control':'no-store','Referrer-Policy':'no-referrer','Set-Cookie':`${previewCookieName(req.headers['x-workbench-preview-origin'])}=${token}; Path=/; HttpOnly; Secure; SameSite=Lax; Max-Age=1800`});res.end();return;
   }
