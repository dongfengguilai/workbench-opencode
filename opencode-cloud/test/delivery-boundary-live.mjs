...
import assert from 'node:assert/strict';import {writeFileSync,readFileSync} from 'node:fs';import {request,login,config} from './delivery-client.mjs';import https from 'node:https';
...
try{
 ...
 const bz=await request('/__platform/source.zip',{cookie:b});assert.equal(bz.status,200);writeFileSync('evidence/delivery-user-b-source.zip',bz.buffer);
 ...
 const ticket=(await request('/__platform/preview/ticket',{method:'POST',cookie:a})).json().url;const path=new URL(ticket).pathname+new URL(ticket).search;
 const claim=await request(path,{preview:true});assert.equal(claim.status,303);pc=claim.headers['set-cookie'][0].split(';')[0];assert.match(claim.headers['set-cookie'][0],/HttpOnly/);assert.match(claim.headers['set-cookie'][0],/SameSite=Lax/);assert.doesNotMatch(claim.headers['set-cookie'][0],/Secure/);
 ...
 const page=await request('/',{preview:true,cookie:pc});assert.equal(page.status,200);assert.match(page.text,/文本统计器/);
 assert.equal((await request('/',{preview:true,cookie:pc,headers:{'Sec-Fetch-Site':'cross-site','Sec-Fetch-Mode':'navigate','Sec-Fetch-Dest':'document'}})).status,200);
 for(const headers of [{Host:'127.0.0.1:8445'},{Origin:'http://127.0.0.1:8444'},{'Sec-Fetch-Site':'cross-site','Sec-Fetch-Mode':'cors','Sec-Fetch-Dest':'empty'}])assert.equal((await request('/',{preview:true,cookie:pc,headers})).status,403);
 assert.equal((await request('/',{preview:true,cookie:pc,method:'POST',headers:{Origin:'https://evil.example'}})).status,403);
 assert.equal((await request('/__platform/source.zip',{preview:true,cookie:pc})).status,403);
 assert.notEqual((await request('/@fs/workspace/project/.env.a05-acceptance',{preview:true,cookie:pc})).status,200);
 assert.notEqual((await request('/@fs/trusted/opencode.json',{preview:true,cookie:pc})).status,200);
 assert.notEqual((await request('/@fs/workspace/project/.git/config',{preview:true,cookie:pc})).status,200);
 ...
 await request('/api/auth/logout',{method:'POST',cookie:a});assert.equal((await request('/',{preview:true,cookie:pc})).status,401);
...
}catch(e){result.status='FAIL';result.error=e.message;throw e;}finally{if(a)await request('/api/auth/logout',{method:'POST',cookie:a});if(b)await request('/api/auth/logout',{method:'POST',cookie:b});writeFileSync('evidence/delivery-boundary-live-result.json',JSON.stringify(result,null,2)+'\n')}
