...
try {
 ...
 assert.match((await app('/',pa)).text,/文件上传下载/);assert.match((await app('/',pb)).text,/WorkBench 新网页/);
 ...
 const pkg='runtime/project/web/package.json';assert.equal(existsSync(pkg),false,'Refusing to overwrite existing user package');
 ...
 writeFileSync('evidence/sidebar-preview-boundary-result.json',JSON.stringify(result,null,2)+'\n');
}
