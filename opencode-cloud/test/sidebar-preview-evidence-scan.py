...
for pattern in ['sidebar-preview*']:
...
result={'status':'PASS' if not hits else 'FAIL','filesScanned':scanned,'credentialHits':hits,'secretValuesPrinted':False};(root/'evidence/sidebar-preview-evidence-scan.json').write_text(json.dumps(result,indent=2)+'\n');print(json.dumps(result));raise SystemExit(bool(hits))
